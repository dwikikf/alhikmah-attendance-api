...
func (r *reportCachePostgres) Get(reportType, classID, periodStart, periodEnd, subject string) (json.RawMessage, error) {
	if subject != "" {
		return nil, nil // Do not cache subject-specific reports to avoid schema change
	}
...
func (r *reportCachePostgres) Set(reportType, classID, periodStart, periodEnd, subject, generatedBy string, data json.RawMessage) error {
	if subject != "" {
		return nil
	}
	query := `
...
func (r *reportCachePostgres) Delete(reportType, classID, periodStart, periodEnd, subject string) error {
	if subject != "" {
		return nil
	}
	query := `
